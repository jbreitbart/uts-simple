...
#define UTS_VERSION "2.1 modified"
...
static char * uts_trees_str[]     = { "Binomial", "Geometric", "Hybrid", "Balanced" };
static char * uts_geoshapes_str[] = { "Linear decrease", "Exponential decrease", "Cyclic", 
 ...
 *   root and binomial distributions towards the leaves.
 */
static tree_t type  = GEO; // Default tree type
static double b_0   = 4.0; // default branching factor at the root
static int   rootId = 0;   // default seed for RNG state at root
 ...
 */
static int    nonLeafBF   = 4;            // m
static double nonLeafProb = 15.0 / 64.0;  // q
 ...
 *  Default parameter values
 */
static int        gen_mx   = 6;      // default depth of tree
static geoshape_t shape_fn = LINEAR; // default shape function (b_i decr linearly)
 ...
 */
static double shiftDepth = 0.5;         

/* compute granularity - number of rng evaluations per tree node */
static int computeGranularity = 1;
...
int uts_paramsToStr(char *strBuf, int ind) {
  ...
  ind += sprintf(strBuf+ind, "UTS - Unbalanced Tree Search %s \n", UTS_VERSION);
  ...
  if (type == BALANCED) {
    ...
    ind += sprintf(strBuf+ind, "        Expected size: %llu nodes, %llu leaves\n",
        (unsigned long) ((pow(b_0, gen_mx+1) - 1.0)/(b_0 - 1.0)) /* geometric series */,
        (unsigned long) pow(b_0, gen_mx));
  }
...
void uts_helpMessage() {
  printf("  UTS - Unbalanced Tree Search %s\n\n", UTS_VERSION);
...
void uts_showStats(int nThreads, double walltime, unsigned long nNodes, unsigned long nLeaves, unsigned long maxDepth) {
    ...
    printf("Wallclock time = %.3f sec, performance = %.0f nodes/sec (%.0f nodes/sec per PE)\n\n",
        walltime, (nNodes / walltime), (nNodes / walltime / nThreads));
}
